...
def save_feature_importance(importance_scores, feature_names, method, prefix, run_id):
    """
    ...
    df.to_csv(f'artifacts/{run_id}/feature selection/{prefix}_{method}_importance.csv', index=False)

def plot_feature_importance(importance_scores, feature_names, method, prefix, run_id):
    """
    ...
    plt.tight_layout()
    plt.savefig(f'artifacts/{run_id}/figures/{prefix}_{method}_importance.png')
    plt.close()

def feature_selection(dev_F, dev_NF, oos_F, oos_NF, feature_names, method, feature_threshold, run_id, ratios=[0.8,0.5,0.2], hidden_activation='relu', dropout=0.1, optimizer='adam', loss='mse', epochs=10, batch_size=32):
    ...
    history_F = autoencoder_F.fit(dev_F, dev_F, epochs=epochs, batch_size=batch_size,
                               validation_data=(oos_F, oos_F), verbose = 0)
    ...
    history_NF = autoencoder_NF.fit(dev_NF, dev_NF, epochs=epochs, batch_size=batch_size,
                                 validation_data=(oos_NF, oos_NF), verbose = 0)
    ...
    save_feature_importance(importance_F, feature_names, method, 'abnormal', run_id)
    save_feature_importance(importance_NF, feature_names, method, 'normal', run_id)
    plot_feature_importance(importance_F, feature_names, method, 'abnormal', run_id)
    plot_feature_importance(importance_NF, feature_names, method, 'normal', run_id)
